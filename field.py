...
fname = r'e:\Users\EYA\Hardangerfjorden\data_Hardangerfjord\plot_1D\BROM_Hfjord_out.nc'
#os.path.split(ask_filename)[1] 

fh =  Dataset(fname) # data from NetCDF will be in "fh" from now 
...
def plot_brom(var_brom,axis):    
    for day in range(0,1370,10):  # plot every 10'th day  to day 1350 (can be "len_time")
        if  day>1290 and day<1370 :
            col = '#ff8800'
        else :
            col = '#a1a7af'

        for n in range(0,10,1): # plot every 1st column in 10 column to avoid zone affected by FF
#       for n in range(0,len_i,2): # plot every 2d column
                #for m in range(0,100):# time_brom-1:
            axis.plot(var_brom[day,:15,n],depth_brom[:15],
                      color =col,alpha = 0.6, # "alpha" is transperancy
                      linewidth = 0.4) 
...
# plot figure with observarions and model for water and sediment     
